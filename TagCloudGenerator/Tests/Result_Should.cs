...
namespace TagCloudGenerator.Tests
{
	...
	public class Result_Should
	{
		...
		{
			var r = Result.Result.Fail<int>("123");
		...
		{
			var res = Result.Result.Of<int>(() => { throw new Exception("123"); });

			res.ShouldBeEquivalentTo(Result.Result.Fail<int>("123"));
		}

		[Test]
		public void ReturnsFailWithCustomMessage_FromResultOf_OnException()
		{
			var res = Result.Result.Of<int>(() => { throw new Exception("123"); }, "42");

			res.ShouldBeEquivalentTo(Result.Result.Fail<int>("42"));
		}

		[Test]
		public void ReturnsOk_FromResultOf_WhenNoException()
		{
			var res = Result.Result.Of(() => 42);

			res.ShouldBeEquivalentTo(Result.Result.Ok(42));
		}

		[Test]
		public void RunThen_WhenOk()
		{
			var res = Result.Result.Ok(42)
				.Then(n => n + 10);
			res.ShouldBeEquivalentTo(Result.Result.Ok(52));
		}

		[Test]
		public void SkipThen_WhenFail()
		{
			var fail = Result.Result.Fail<int>("ошибка");
			var called = false;
			fail.Then(n =>
			{
				called = true;
		...
		{
			var res =
				Result.Result.Ok("1358571172")
					.Then(int.Parse)
					.Then(i => Convert.ToString(i, 16))
					.Then(hex => Guid.Parse(hex + hex + hex + hex));
			res.ShouldBeEquivalentTo(Result.Result.Ok(Guid.Parse("50FA26A450FA26A450FA26A450FA26A4")));
		}

		[Test]
		public void RunThen_WhenOk_ComplexScenario()
		{
			var parsed = Result.Result.Ok("1358571172").Then(int.Parse);
			var res = parsed
				.Then(i => Convert.ToString(i, 16))
				.Then(hex => parsed.GetValueOrThrow() + " -> " + Guid.Parse(hex + hex + hex + hex));
			res.ShouldBeEquivalentTo(Result.Result.Ok("1358571172 -> 50fa26a4-50fa-26a4-50fa-26a450fa26a4"));
		}

		[Test]
		public void ReplaceError_IfFail()
		{
			Result.Result.Fail<None>("error")
				.ReplaceError(e => "replaced")
				.ShouldBeEquivalentTo(Result.Result.Fail<None>("replaced"));
		}
		...
		public void ReplaceError_DoNothing_IfSuccess()
		{
			Result.Result.Ok(42)
				.ReplaceError(e => "replaced")
				.ShouldBeEquivalentTo(Result.Result.Ok(42));
		}
		...
		public void ReplaceError_DontReplace_IfCalledBeforeError()
		{
			Result.Result.Ok(42)
				.ReplaceError(e => "replaced")
				.Then(n => Result.Result.Fail<int>("error"))
				.ShouldBeEquivalentTo(Result.Result.Fail<int>("error"));
		}

		[Test]
		public void RefineError_AddErrorMessageBeforePreviousErrorText()
		{
			var calculation = Result.Result.Fail<None>("No connection");
			calculation
				.RefineError("Posting results to db")
				.ShouldBeEquivalentTo(Result.Result.Fail<None>("Posting results to db. No connection"));
		}
